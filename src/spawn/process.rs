    ...
    pub mod sync {
        ...
        fn spawn_posix(
            ...
            let status: Status = 'blk: {
                ...
                // OHOS no_orphans: there is no parent watch in this loop. The
                // signalfd/pidfd wait path is Linux-only and unavailable here,
                // so the kernel's PR_SET_PDEATHSIG (armed at spawn) is the
                // parent-death guarantee and is left in place: trading it for
                // a pidfd that nothing polls would drop `--no-orphans`
                // entirely. The cost is that the cleanup defer does not run in
                // the SIGKILL case, matching upstream Linux behaviour.
