...
#if defined(__OHOS__)
// OHOS SDK Clang 15 has bugs with consteval, use constexpr instead
template<typename... T>
requires(Detail::isCharArray<std::remove_cvref_t<T>> && ...)
constexpr auto concatCStrings(T&&... nullTerminatedCharArrays)
{
...
constexpr auto listSeparatorForIndex()
{
...
constexpr auto joinCStringsAsList(std::index_sequence<i0, indices...>, T&& first, Rest&&... rest)
{
...
template<typename... T>
requires(Detail::isCharArray<std::remove_cvref_t<T>> && ...)
constexpr auto joinCStringsAsList(T&&... nullTerminatedCharArrays)
{
