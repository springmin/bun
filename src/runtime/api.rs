...
// Gives an exec'd `node` child a working os.userInfo(): the runtime patch
// (see js_bun_spawn_bindings.rs's OHOS PWD block for the sibling workaround)
// only covers this process, not a spawned child.
