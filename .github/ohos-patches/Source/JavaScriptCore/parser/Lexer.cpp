...
size_t parsedLength;
tokenData->doubleValue = parseDouble(std::span<const Latin1Character>{m_buffer8.span()}, parsedLength);
if (token == INTEGER)
...
tokenData->pattern = makeRightSizedIdentifier(std::span<const char16_t>{m_buffer16.span()}, charactersOredTogether);
m_buffer16.shrink(0);
