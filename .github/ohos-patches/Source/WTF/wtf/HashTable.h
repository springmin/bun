...
#define CHECK_HASHTABLE_USE_AFTER_DESTRUCTION 1
    ...
    // Useful when the key type is WeakPtr - C++17 compatible SFINAE
    template<typename KeyTraitsT = KeyTraits, typename = std::enable_if_t<KeyTraitsT::hasIsWeakNullValueFunction>>
    size_t computeSize() const;
    template<typename KeyTraitsT = KeyTraits, typename = std::enable_if_t<KeyTraitsT::hasIsWeakNullValueFunction>>
    bool isEmptyIgnoringNullReferences() const;
    template<typename KeyTraitsT = KeyTraits, typename = std::enable_if_t<KeyTraitsT::hasIsWeakNullValueFunction>>
    void removeWeakNullEntries() const;
...
template<typename Key, typename Value, typename Extractor, typename HashFunctions, typename Traits, typename KeyTraits, typename Malloc>
template<typename KeyTraitsT, typename>
