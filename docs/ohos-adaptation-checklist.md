...
| `test/js/bun/module-graph/module-graph-workers.test.ts` | ⚠️ 仍余 4 例（node Worker **handler** 单元格）：每个场景多留 **1 个 socket**（插桩定位 `fd 17:socket:[...]`；最小 worker/BroadcastChannel/MessageChannel 探针均 0 泄漏）→ 指向 graph 的 worker-handler 释放路径，待专项 | 上游改 ModuleGraph handler/worker 释放时复测；其余 475 例通过 |
