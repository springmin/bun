...
| `test/js/bun/module-graph/` | 上游 ModuleGraph（per-graph context）新增测试：`module-graph-isolation`（2-3 例，disposed graph 的 file-sink/子进程 stdio 管道未全关）与 `module-graph-workers`（4 例，node Worker handler 场景的 results 深比较 +2）在 OHOS 上确定性失败；其余 410/475 例通过 | ⚠️ **已知差异**：新特性的细粒度资源回收语义（socketpair 回退路径的 fd 生命周期）待专项分析；上游改 ModuleGraph 时复测 |
