...
function emitNestedCmake(
  ...
  //   android/ohos   → add -fPIC regardless (their loaders are PIE-only)
  ...
  if (spec.pic || cfg.abi === "android" || cfg.ohos) {
    if (!cfg.windows) {
      cflags += " -fPIC";
      cxxflags += " -fPIC";
    }
...
function emitDirect(
  ...
  // (flags.ts). spec.pic → -fPIC, and Android/OHOS always (PIE-only
  // platforms); otherwise on unix undo the building clang's PIC/PIE default to
  // match the non-PIE final binary. Not only apple-clang has one: distro
  // clangs (Arch, Fedora, Alpine, Homebrew) default to PIE, which made every
  // dep PIC when built there (.data.rel.ro 541 KB instead of 109 KB, 4-7% more
  // pages touched at startup) while CI's apt.llvm.org clang did not.
