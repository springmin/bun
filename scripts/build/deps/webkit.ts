...
function webkitSourceIncludes(cfg: Config): string[] {
  ...
  // OHOS: FFI headers (BunFFI.h, FFISignature.h, etc.) live in the source
  // tree's ffi/ subdirectory but are included as <JavaScriptCore/*.h> from
...
export const webkit: Dependency = {
  ...
  provides: cfg => {
    ...
    const libs: string[] = [];
    // OHOS: static cross-built ICU libs from $BUN_OHOS_ICU_DIR (set up by
    // the OHOS build environment); system ICU is not available on-device.
